...
import { chapterMetrics, contentToParagraphs } from './story-content.js';
import { enforceMinimumAgeRating, storyGenreName, storyTagKind, type StoryAgeRating } from './story-taxonomy.js';
...
export interface CreateStoryParams { authorId: string; title: string; synopsis: string; genres: string[]; tags: string[]; isMature?: boolean; ageRating?: 'all' | '11' | '13' | '16' | '18'; creationMethod?: 'human' | 'ai_assisted' | 'ai_generated'; coverColor?: string; status?: 'draft' | 'published' }
export interface UpdateStoryParams { authorId: string; storyId: string; title?: string; synopsis?: string; genres?: string[]; tags?: string[]; isMature?: boolean; ageRating?: 'all' | '11' | '13' | '16' | '18'; creationMethod?: 'human' | 'ai_assisted' | 'ai_generated'; coverColor?: string | null }
export interface CreateChapterParams { storyId: string; title: string; content: unknown; status?: 'draft' | 'published' }
export interface UpdateChapterParams { chapterId: string; authorId: string; title: string; content: unknown; expectedVersion: number }
...
async function resolveGenres(tx: Prisma.TransactionClient, names: string[]) {
  const unique = [...new Map(names.map((name) => {
    const canonical = storyGenreName(name);
...
export class WriterRepository {
  async getAllStories(includeArchived = false): Promise<StorySummary[]> {
    ...
    const stories = await prisma.story.findMany({ where: includeArchived ? {} : { status: { not: 'archived' } }, orderBy: { updatedAt: 'desc' }, take: 200, include: { author: { include: { profile: true } }, genres: { include: { genre: true } }, tags: { include: { tag: true } }, _count: { select: { chapters: true } } } });
    return stories.map((story) => ({ id: story.id, title: story.title, author: story.attributionName ?? story.author.profile?.displayName ?? story.author.username, authorUsername: story.author.username, synopsis: story.synopsis, genre: story.genres[0]?.genre.name ?? 'General', genres:story.genres.map((item)=>item.genre.name),tags:story.tags.map((item)=>({name:item.tag.name,kind:item.tag.kind})),languageCode:story.languageCode,ageRating:story.ageRating as StoryAgeRating,creationMethod: story.creationMethod,status: story.status, chapterCount: story._count.chapters, isMature: story.isMature, coverColor: story.coverUrl ?? '#855300', updatedAt: story.updatedAt.toISOString() }));
  }
  ...
  async getUserStories(authorId: string, includeArchived = false): Promise<StorySummary[]> {
    ...
    const stories = await prisma.story.findMany({ where: { authorId, ...(includeArchived ? {} : { status: { not: 'archived' } }) }, orderBy: { updatedAt: 'desc' }, take: 200, include: { author: { include: { profile: true } }, genres: { include: { genre: true } }, tags: { include: { tag: true } }, _count: { select: { chapters: true } } } });
    return stories.map((story) => ({ id: story.id, title: story.title, author: story.attributionName ?? story.author.profile?.displayName ?? story.author.username, authorUsername: story.author.username, synopsis: story.synopsis, genre: story.genres[0]?.genre.name ?? 'General', genres:story.genres.map((item)=>item.genre.name),tags:story.tags.map((item)=>({name:item.tag.name,kind:item.tag.kind})),languageCode:story.languageCode,ageRating:story.ageRating as StoryAgeRating,creationMethod: story.creationMethod,status: story.status, chapterCount: story._count.chapters, isMature: story.isMature, coverColor: story.coverUrl ?? '#855300', updatedAt: story.updatedAt.toISOString() }));
  }
  ...
  async getUserStory(authorId: string, storyId: string) {
    ...
    return {id:story.id,title:story.title,author:story.attributionName??story.author.profile?.displayName??story.author.username,authorUsername:story.author.username,synopsis:story.synopsis,genre:story.genres[0]?.genre.name??'General',genres:story.genres.map((item)=>item.genre.name),tags:story.tags.map((item)=>({name:item.tag.name,kind:item.tag.kind})),languageCode:story.languageCode,ageRating:story.ageRating as StoryAgeRating,creationMethod: story.creationMethod,status:story.status,chapterCount:story.chapters.length,isMature:story.isMature,coverColor:story.coverUrl??'#855300',chapters:story.chapters.map((chapter)=>({...chapter,updatedAt:chapter.updatedAt.toISOString()}))};
  }
  ...
  async createStory(params: CreateStoryParams): Promise<StorySummary> {
    ...
    const ageRating = enforceMinimumAgeRating(
      params.ageRating ?? (params.isMature ? '18' : 'all'),
      params.tags,
    );
    if (!(await checkDatabaseConnection())) { const story:StorySummary={id:`story-${Date.now()}`,title:params.title,author:params.authorId,authorUsername:params.authorId,synopsis:params.synopsis,genre:primaryGenre,genres:params.genres,tags:params.tags.map((name)=>({name,kind:storyTagKind(name)??'theme'})),languageCode:'es',ageRating,creationMethod:params.creationMethod??'human',status,chapterCount:0,isMature:ageRating==='18',coverColor:params.coverColor??'#855300'};if(status==='published')storyFixtures.unshift(story);const list=offlineStoriesByAuthor.get(params.authorId)??[];list.unshift(story);offlineStoriesByAuthor.set(params.authorId,list);return story; }
    const story = await prisma.$transaction(async (tx) => {
      const [genres, tags] = await Promise.all([
        resolveGenres(tx, params.genres),
        resolveTags(tx, params.tags),
      ]);
      const data: Prisma.StoryCreateInput = {
        ...
        ageRating,
        creationMethod: params.creationMethod ?? 'human',
      ...
      return tx.story.create({
        data,
        include: { author: { include: { profile: true } }, genres: { include: { genre: true } }, tags: { include: { tag: true } } },
      });
    });
    return{id:story.id,title:story.title,author:story.author.profile?.displayName??story.author.username,authorUsername:story.author.username,synopsis:story.synopsis,genre:story.genres[0]?.genre.name??primaryGenre,genres:story.genres.map((item)=>item.genre.name),tags:story.tags.map((item)=>({name:item.tag.name,kind:item.tag.kind})),languageCode:story.languageCode,ageRating:story.ageRating as StoryAgeRating,creationMethod: story.creationMethod,status:story.status,chapterCount:0,isMature:story.isMature,coverColor:story.coverUrl??'#855300'};
  }

  async updateStory(params:UpdateStoryParams){
    if (!(await checkDatabaseConnection())) {
      const story = (offlineStoriesByAuthor.get(params.authorId) ?? [])
        .find((item) => item.id === params.storyId);
      if (!story) return null;
      const tagNames = params.tags ?? (story.tags ?? []).map((tag) => tag.name);
      const requestedAge = params.ageRating
        ?? (params.isMature ? '18' : story.ageRating ?? (story.isMature ? '18' : 'all'));
      const ageRating = enforceMinimumAgeRating(requestedAge, tagNames);
      if (params.title !== undefined) story.title = params.title;
      if (params.synopsis !== undefined) story.synopsis = params.synopsis;
      if (params.genres !== undefined) {
        story.genres = params.genres;
        story.genre = params.genres[0] ?? 'General';
      }
      if (params.tags !== undefined) {
        story.tags = params.tags.map((name) => ({ name, kind: storyTagKind(name) ?? 'theme' }));
      }
      if (params.creationMethod !== undefined) story.creationMethod = params.creationMethod;
      if (params.coverColor !== undefined) story.coverColor = params.coverColor ?? '#855300';
      story.ageRating = ageRating;
      story.isMature = ageRating === '18';
    ...
    await prisma.$transaction(async(tx)=>{
      const [genres,tags]=await Promise.all([params.genres?resolveGenres(tx,params.genres):null,params.tags?resolveTags(tx,params.tags):null]);
      ...
      await tx.story.update({where:{id:params.storyId},data:{...(params.title!==undefined?{title:params.title}:{}),...(params.synopsis!==undefined?{synopsis:params.synopsis}:{}),...(params.ageRating!==undefined || params.tags!==undefined || params.isMature!==undefined?{ageRating,isMature:ageRating==='18'}:{}),...(params.creationMethod!==undefined?{creationMethod:params.creationMethod}:{}),...(params.coverColor!==undefined?{coverUrl:params.coverColor}:{})}});
    });
    return this.getUserStory(params.authorId,params.storyId);
  }

  async createChapter(params: CreateChapterParams) {
    const status=params.status??'published';const paragraphs=contentToParagraphs(params.content);const plainText=paragraphs.join('\n\n');const count=chapterMetrics(plainText);
    ...
    const chapter=await prisma.$transaction(async(tx)=>{
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${params.storyId}, 0))`;
      const last=await tx.chapter.findFirst({where:{storyId:params.storyId},orderBy:{position:'desc'},select:{position:true}});
      const position=(last?.position??0)+1;
      const created=await tx.chapter.create({data:{storyId:params.storyId,title:params.title,slug:`${slugify(params.title,'capitulo')}-${position}`,position,status,contentJson:params.content as Prisma.InputJsonValue,plainText,...count,...(status==='published'?{publishedAt:new Date()}:{})}});
      if(status==='published')await tx.story.update({where:{id:params.storyId},data:{publishedChapterCount:{increment:1},wordCount:{increment:count.wordCount}}});
      return created;
    });
    return{...chapter,content:params.content};
  }
  ...
  async deleteChapter(authorId: string, storyId: string, chapterId: string) {
    ...
    return prisma.$transaction(async (tx) => {
      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${storyId}, 0))`;
      ...
      await tx.$executeRaw`
        ...
        WHERE "story_id" = CAST(${storyId} AS uuid) AND "position" > ${chapter.position}
      `;
      await tx.$executeRaw`
        UPDATE "chapters"
        SET "position" = -"position" - 1
        WHERE "story_id" = CAST(${storyId} AS uuid) AND "position" < 0
      `;
