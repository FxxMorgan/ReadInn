...
export default function ExplorePage() {
  ...
  const [creationMethod, setCreationMethod] = useState('all');
  ...
  const activeFilters = genres.length + tags.length + Number(mature !== 'exclude') + Number(creationMethod !== 'all') + Number(minChapters > 0) + Number(minRating > 0);
  useEffect(() => {
    const initialQuery = new URLSearchParams(window.location.search).get('query')?.trim();
    if (initialQuery) setQuery(initialQuery);
  }, []);
  const searchParams = useMemo(() => {
    const params = new URLSearchParams({ limit: '50', sort, mature, creationMethod });
  ...
  }, [creationMethod, genres, mature, minChapters, minRating, query, sort, tags]);
  ...
  function clearFilters() {
    setGenres([]); setTags([]); setSort('recent'); setMature('exclude'); setCreationMethod('all'); setMinChapters(0); setMinRating(0); setShowAllResults(false);
  }
  ...
  return (
    <div className="page discovery-page">
      ...
      {showFilters && <section className="advanced-filters" aria-label="Filtros avanzados">
        ...
        <div className="filter-row"><label>Orden<select value={sort} onChange={(event) => setSort(event.target.value)}>{taxonomy.sortOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>Contenido<select value={mature} onChange={(event) => setMature(event.target.value)}><option value="exclude">Sin contenido adulto</option><option value="include">Todo</option><option value="only">Solo adulto</option></select></label><label>Origen<select value={creationMethod} onChange={(event) => setCreationMethod(event.target.value)}><option value="all">Cualquier origen</option><option value="human">Creada por autor</option><option value="ai_assisted">Asistida por IA</option><option value="ai_generated">Generada por IA</option></select></label><label>Capitulos minimos<input type="number" min={0} value={minChapters} onChange={(event) => setMinChapters(Math.max(0, Number(event.target.value)))} /></label><label>Valoracion minima<select value={minRating} onChange={(event) => setMinRating(Number(event.target.value))}><option value={0}>Cualquiera</option><option value={3}>3+</option><option value={4}>4+</option><option value={4.5}>4.5+</option></select></label></div>
